...
if __name__ == '__main__':
    ...
    for Set in results:
        ...
        minY = float('inf')
        maxY = float('-inf')
        for alg in sorted(results[Set], key=lambda item: (len(item), item)):
            print(f'. Alg: {alg}')
            ...
            for Seq in sorted(results[Set][alg]):
                ...
                y.append(hvMean)
                yerr.append(hvCI)
                if minY > hvMean-hvCI: minY = hvMean-hvCI
                if maxY < hvMean+hvCI: maxY = hvMean+hvCI
            conf = next(config)
            plt.errorbar(x, y, yerr=yerr, label=alg, marker=conf[0], color=conf[1])
        plt.xlim([1, 25])
        plt.ylim([minY, maxY])
