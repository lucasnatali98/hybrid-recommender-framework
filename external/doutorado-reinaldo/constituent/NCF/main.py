...
if config.model == 'NeuMF-pre': optimizer = optim.SGD(model.parameters(), lr=args.lr)
...
def predict_test_file(model):
    ...
    # dataset = "Amazon"
    # sample = "tuning70"
    usermap_FILE = open(currentdir + "data/ratings" + dataset + sampleTrain + ".usermap", "r")
    itemmap_FILE = open(currentdir + "data/ratings" + dataset + sampleTrain + ".itemmap", "r")
    user_map = {}
    item_map = {}
    ...
    to_predict_file = open(main_path + "Sample" + sampleTest + ".test", "r")
    m_users = []
    m_items = []
    i_users = []
    i_items = []
    ...
    predDir = os.path.dirname(predictionfile)
    if not os.path.exists(predDir): os.makedirs(predDir)
    ...
    f = open(predictionfile.replace(".tsv", ".eval.txt"), 'w+')
