...
measures = [ 'NDCG', 'EPD', 'EILD',
             ...
             'Fairness-NDCG', 'Fairness-EPD', 'Fairness-EILD' ]
...
def getData(filePattern):
    global ignoreRisk, ignoreIndED, ignoreRisk, bestFilter
    ...
    for fileName in files:
        ...
        if 'Fair' in alg: continue
        if len(bestFilter) > 0 and alg not in bestFilter: continue
...
if __name__ == '__main__':
    ...
    bestFilter         = [] if len(sys.argv) <= 10 else sys.argv[10].split(";")
