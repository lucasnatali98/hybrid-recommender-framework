...
class ConfidenceIntervalMean(ConfidenceInterval):
    def __init__(self, parameters: dict) -> None:
        ...
        default_keys = set()
        parameters = process_parameters(parameters, default_keys)
        self.alpha = parameters.get('alpha', 0.95)
...
class ConfidenceIntervalLinearDifference(ConfidenceInterval):
    ...
    def get_results(self, sample_data, **kwargs):
        ...
        #O valor 0.3 está chumbado e precisa ser alterado para sua variável correta
        criticalvalue = stats.t.ppf(q=1 - 0.3 / 2, df=len_no_obs - 1)
